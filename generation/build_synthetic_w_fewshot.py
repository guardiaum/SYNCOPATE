...
if __name__ == '__main__':
    ...
    dir2synthetic_path = "../datasets/synthetic/fewshot/{0}".format(dataset_ref)
    ...
    with open(dest_synthetic_samples, type, newline='') as f:
        ...
        while count_saved_sentences != number_of_synthetic_samples:
            count_iteration += 1
            print("="*100)
            prompt = get_random_prompt(dataset_ref, n_samples=5)
            print("[PROMPT] : \n{0}\n".format(prompt))

            gen = generate(tokenizer, model, device, prompt, max_length=500)
            sentence, tuples = extract_from_gen_fewshot(gen, n_samples=5)
